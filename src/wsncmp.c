...
benchmark_pre(void *tsd)
{
	...
	if (ts->ts_once++ == 0) {
		...
		LM_CHK((ts->ts_a = malloc(opts * sizeof (wchar_t)+ 1)) != NULL);
		LM_CHK((ts->ts_b = malloc(opts * sizeof (wchar_t)+ 1 + unaligned)) != NULL);
		ts->ts_b += unaligned;
...
benchmark(void *tsd, result_t *res)
{
	...
	wchar_t			*src = ts->ts_a;
	wchar_t			*src2 = ts->ts_b;

	*sum += wsncmp(src, src2, opts);
